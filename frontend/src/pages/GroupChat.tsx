...
const GroupChat = () => {
  ...
  useEffect(() => {
    ...
    socket.current.on("connect", () => {
      console.log("✅ Connected to Group Chat Socket.IO")

      socket.current?.emit("join", { room: groupName, user: username })
      socket.current?.emit("group-join", { room: groupName, user: username })
    })

    socket.current.on(`Recieve-Message-${groupName}`, (msg: WSMessage) => {
      saveMessage(msg)
    })

    socket.current.on(`Group-Join-${groupName}`, (msg: WSMessage) => {
      showInfo(msg.data)
      setOnlineUsers((prev) =>
        msg.user && !prev.includes(msg.user) ? [...prev, msg.user] : prev
      )
    })
    ...
    return () => {
      if (socket.current) {
        socket.current.emit("leave", { room: groupName, user: username })
